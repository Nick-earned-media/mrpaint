  ...
  if (c?.skipped) {
    ...
    if (c.opportunities.length === 0) {
      ...
      for (const op of c.opportunities.slice(0, 5)) {
        const tags = [];
        if (op.volume) tags.push(`${op.volume}/mo`);
        if (op.isCommercial) tags.push("💰");
        if (op.cpc) tags.push(`CPC $${op.cpc.toFixed(2)}`);
        const meta = tags.length ? ` (${tags.join(", ")})` : "";
        m.push(`• "${op.keyword}" — ${op.competitor} #${op.competitor_position}${meta}`);
      }
    }
