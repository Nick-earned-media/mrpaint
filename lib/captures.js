...
async function appendMediaToCapture(id, mediaItem) {
  // Atomic JSONB append via the Postgres RPC defined in migration 003.
  // Read-modify-write in JS would race when Twilio fires multiple webhooks
  // for a batch upload within ~100ms (3-photo batches were landing as 2).
  const { error } = await client().rpc("append_capture_media", {
    p_id: id, p_item: mediaItem,
  });
  if (error) throw new Error(`appendMedia rpc: ${error.message}`);
}

async function appendPendingMedia(id, mediaItem) {
  // Atomic — see appendMediaToCapture. Also flips status to
  // 'awaiting_same_or_new' and resets the reminder flags.
  const { error } = await client().rpc("append_capture_pending_media", {
    p_id: id, p_item: mediaItem,
  });
  if (error) throw new Error(`appendPendingMedia rpc: ${error.message}`);
}
