 ...
 * mediaUrl    — Twilio URL (fetched with auth) or any URL
 * mediaContentType — MIME type hint
 * bufferOverride   — pre-fetched Buffer; skips the URL fetch entirely
 *                    (used by web/Slack channels that already have the bytes)
 * Returns { ok, text, error }.
 */
async function transcribeTwilioAudio(mediaUrl, mediaContentType, bufferOverride) {
  ...
  try {
    const { buf, contentType } = bufferOverride
      ? { buf: bufferOverride, contentType: mediaContentType || "audio/webm" }
