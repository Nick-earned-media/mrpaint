...
const REQUEST_COMPETITOR_TOOL = {
  ...
  description:
    "Add a competitor to the client's whitelist (capped at 8). Use when the user asks to add, track, or watch a specific local competitor business — e.g. 'add Cairns Painting Contractors as a competitor', 'track cairnspainter.com.au', 'I want to watch [name]'. ONLY use for specific local businesses the client names — do NOT add marketplaces (Airtasker, Hipages), directories, or major brands. Returns success or 'at cap' status. Always fires a Slack notification to Nick so he's aware of the change.",
  input_schema: {
    type: "object",
    properties: {
      name: { type: "string", description: "Display name of the competitor (e.g. 'Cairns Painting Contractors')" },
      domain: { type: "string", description: "Competitor's domain (e.g. 'cairnspaintingcontractors.com.au') — strip http:// and www." },
    },
...
async function runRequestCompetitor({ name, domain }, ctx) {
  const { MAX_COMPETITORS, isBrandAggregator, normalizeDomain } = require("./competitor-filter.js");
  const { notifyCompetitorChange } = require("./slack.js");
  if (!name || !domain) return "Both name and domain are required.";
  const cleanDomain = normalizeDomain(domain);
  if (!cleanDomain) return "Invalid domain.";
  if (isBrandAggregator(cleanDomain)) {
    return `${name} (${cleanDomain}) is a marketplace/aggregator — those aren't tracked as competitors. Suggest a specific local business instead.`;
  }
  // Count current active competitors
  const { count, error: countErr } = await supa()
    .from("competitors")
    .select("id", { count: "exact", head: true })
    .eq("client_id", ctx.clientId)
    .eq("active", true);
  if (countErr) return `Couldn't check current count: ${countErr.message}`;
  if ((count || 0) >= MAX_COMPETITORS) {
    return `Already at the ${MAX_COMPETITORS}-competitor cap. Ask which one to remove first (use list_competitors to see the current list).`;
  }
  // Check for existing entry on the same domain
  ...
  if (existing && existing.active) {
    return `${name} (${cleanDomain}) is already in the whitelist.`;
  }
  if (existing && !existing.active) {
    // Reactivate
    const { error } = await supa()
      ...
      .eq("id", existing.id);
    if (error) return `Reactivate failed: ${error.message}`;
  } else {
    ...
    const { error } = await supa()
      ...
      .insert({
        ...
        domain: cleanDomain,
        active: true,
        notes: `Added via WhatsApp ${today}`,
      });
  ...
  return `Added ${name} (${cleanDomain}) to the competitor whitelist. Now ${newCount}/${MAX_COMPETITORS}. Nick has been notified.`;
}
...
const REMOVE_COMPETITOR_TOOL = {
  ...
  description:
    "Remove a competitor from the client's whitelist. Use when the user asks to stop tracking, remove, or drop a competitor — e.g. 'stop tracking Cairns Painter', 'remove [name]'. Accepts either the name or the domain. Soft-deletes (sets active=false) so the history is preserved. Fires a Slack notification to Nick.",
  input_schema: {
    type: "object",
    properties: {
...
async function runRemoveCompetitor({ name_or_domain }, ctx) {
  const { normalizeDomain } = require("./competitor-filter.js");
  const { notifyCompetitorChange } = require("./slack.js");
  if (!name_or_domain) return "name_or_domain is required.";
  const q = String(name_or_domain).trim();
  const clean = normalizeDomain(q);
  // Try match on domain first, then name
  ...
  if (!match) return `Couldn't find '${q}' in the active competitor list. Use list_competitors to see what's there.`;
  const { error } = await supa()
    .from("competitors")
    .update({ active: false })
    .eq("id", match.id);
  ...
  return `Removed ${match.name} (${match.domain}) from the competitor whitelist. Now ${count || 0}/${MAX_COMPETITORS}. Nick has been notified.`;
}
