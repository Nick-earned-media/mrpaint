...
// ─── tool: flag_for_nick — generic "send a note to Nick" Slack ping ──────

const FLAG_FOR_NICK_TOOL = {
  name: "flag_for_nick",
  description:
    "Send a note to Nick (the agency operator) via Slack about something the user wants actioned by a human — e.g. 'flag a speed audit', 'flag a content rewrite', 'queue a meta description rewrite', 'note that the client wants X added to the report'. Use this any time you've OFFERED to 'flag X for Nick' / 'send a note to Nick' / 'let Nick know about Y' and the user has confirmed. ALWAYS use this when you said you'd flag something — do not silently switch to a different topic or run an analysis tool instead.",
  input_schema: {
    type: "object",
    properties: {
      topic: { type: "string", description: "Short subject line of what you're flagging (e.g. 'Speed audit for /painter-cairns/', 'Content rewrite for /painter-trinity-beach/', 'Add monthly competitor section to report')." },
      details: { type: "string", description: "Concrete details Nick needs — relevant URLs, the specific issue, anything the user said that gives context." },
      urgency: {
        type: "string",
        enum: ["low", "normal", "high"],
        description: "low = housekeeping; normal = this week; high = needs Nick's attention now",
      },
    },
...
async function runFlagForNick(input, ctx) {
  const { postToSlack } = require("./slack.js");
  const topic = String(input.topic || "").trim();
  const details = String(input.details || "").trim();
  const urgency = String(input.urgency || "normal").toLowerCase();
  if (!topic || !details) return "Need both a topic and details to flag for Nick — describe what you want him to look at.";

  const emoji = urgency === "high" ? "🚩" : urgency === "low" ? "📝" : "📌";
  const clientName = ctx.clientRow?.display_name || "client";
  const lines = [
    `${emoji} *${clientName}* — *${topic}*`,
    details,
    `_Urgency: ${urgency}_`,
  ];
  await postToSlack({ text: lines.join("\n") });
  return "Flagged for Nick. He'll see it in Slack and pick it up.";
}
...
const READ_SITE_PAGE_TOOL = {
  name: "read_site_page",
  description:
    "Fetch and read the actual text content of any webpage — the client's site OR a competitor's — so you can analyse headings, body copy, keyword usage, and on-page structure. Use when: 'what's on my [page]?', 'how can I improve [page]?', 'is [page] optimised for [keyword]?', 'what's on their homepage?', 'what are competitors doing on-page?'. For client pages pass the path (e.g. '/industrial-painting/'). For competitor pages pass the full URL (e.g. 'https://www.petespainting.com.au'). Always pass a keyword when auditing for ranking potential. NEVER say 'probably' about page content — fetch it.",
  input_schema: {
    type: "object",
    properties: {
      path: {
        type: "string",
        description: "Client site path (e.g. '/industrial-painting/') OR a full competitor URL (e.g. 'https://www.cairnspaintingcontractors.com/commercial-painting/'). Full URLs take precedence.",
      },
      keyword: {
        type: "string",
        description: "Optional — keyword to audit the page against (e.g. 'industrial painter Cairns'). Notes keyword density and missing usage.",
      },
    },
    required: ["path"],
  },
...
async function runReadSitePage({ path, keyword }, ctx) {
  // Accept full URLs for competitor pages; otherwise resolve against client domain.
  const url = /^https?:\/\//i.test(path)
    ? path
    : (process.env.PUBLIC_BASE_URL || "https://mrpaint.com.au").replace(/\/$/, "") + "/" + path.replace(/^\//, "");

  let html;
  try {
    const resp = await fetch(url, {
      headers: { "User-Agent": "MrPaintBot/1.0 (internal page audit)" },
      signal: AbortSignal.timeout(10000),
    });
    if (!resp.ok) return `Could not fetch ${url} — HTTP ${resp.status}`;
    html = await resp.text();
  } catch (err) {
    return `Fetch failed for ${url}: ${err.message}`;
  }

  // Strip script/style/nav/header/footer noise, extract meaningful content.
  const text = html
  ...
  // Extract title and headings from raw HTML for structure analysis.
  ...
  // Keyword density check if keyword provided.
  let kwNote = "";
  if (keyword) {
    const kw = keyword.toLowerCase();
    const bodyLower = text.toLowerCase();
    const count = (bodyLower.match(new RegExp(kw.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g")) || []).length;
    const wordCount = text.split(/\s+/).length;
    const density = wordCount > 0 ? ((count / wordCount) * 100).toFixed(2) : "0";
    kwNote = `\n\nKeyword audit — "${keyword}": appears ${count} time(s) in body text (${density}% density, ~${wordCount} words total).`;
    if (count === 0) kwNote += " NOT FOUND on page — this is likely a major ranking gap.";
  }

  const cap = 3000;
  const bodySnippet = text.length > cap ? text.slice(0, cap) + `… [truncated — ${text.length} chars total]` : text;

  return [
    `Page: ${url}`,
    `Title tag: ${title || "(none found)"}`,
    `H1s: ${h1s.length ? h1s.join(" | ") : "(none)"}`,
    `H2s: ${h2s.length ? h2s.slice(0, 6).join(" | ") : "(none)"}`,
    `H3s: ${h3s.length ? h3s.slice(0, 6).join(" | ") : "(none)"}`,
    kwNote,
    `\nBody text:\n${bodySnippet}`,
  ].join("\n");
}

// ─── tool: get_peec_visibility ───────────────────────────────────────────
// LLM visibility via Peec.ai — who's being mentioned in AI responses,
// at what position, with what sentiment, and which sources AI models cite.

const PEEC_VISIBILITY_TOOL = {
  name: "get_peec_visibility",
  description:
    "Pull LLM visibility data from Peec.ai — how often MrPaint and competitors appear in AI-generated responses (ChatGPT, Gemini, Perplexity, etc.), at what position, with what sentiment, and which source domains AI models are citing. Use when the user asks: 'how am I showing up in AI?', 'am I being mentioned in ChatGPT?', 'what's my LLM visibility?', 'which AI engines mention me?', 'what sites is AI pulling from?', 'how do I compare to competitors in AI search?'. Returns brand mention counts, avg position, share of voice, sentiment, and top cited source domains — broken down by AI model if requested.",
  input_schema: {
    type: "object",
    properties: {
      days: {
        type: "number",
        description: "Lookback period in days (default 30). Use 7 for 'this week', 30 for 'this month', 90 for 'last quarter'.",
      },
      breakdown: {
        type: "string",
        enum: ["overall", "by_model", "sources"],
        description: "overall = single row per brand; by_model = split by ChatGPT/Gemini/Perplexity/etc; sources = which domains AI is citing.",
      },
    },
  },
};

async function runGetPeecVisibility({ days = 30, breakdown = "overall" }, ctx) {
  const { getBrandVisibility, getSourceDomains, getBrandsByModel } = require("./peec.js");

  if (breakdown === "sources") {
    const r = await getSourceDomains({ days, limit: 15 });
    if (!r.ok) return `Peec.ai source domains failed: ${r.error}`;
    const rows = r.data?.rows || r.data?.data || [];
    if (!rows.length) return `No source domain data from Peec.ai for the last ${days} days.`;
    const lines = [`Top domains AI models are citing (last ${days}d):`];
    for (const row of rows.slice(0, 15)) {
      const domain = row.domain || row.url || row.name || JSON.stringify(row).slice(0, 60);
      const mentions = row.mention_count ?? row.mentions ?? "?";
      lines.push(`  ${domain} — ${mentions} mentions`);
    }
    return lines.join("\n");
  }

  const fetcher = breakdown === "by_model" ? getBrandsByModel : getBrandVisibility;
  const r = await fetcher({ days });
  if (!r.ok) return `Peec.ai brand visibility failed: ${r.error}`;

  const rows = r.data?.rows || r.data?.data || [];
  if (!rows.length) return `No brand visibility data from Peec.ai for the last ${days} days. The project may not have any tracked prompts returning results yet.`;

  const lines = [`LLM brand visibility — Peec.ai (last ${days}d):`];
  if (breakdown === "by_model") {
    // Group by brand then show per-model breakdown
    const byBrand = {};
    for (const row of rows) {
    ...
    for (const [brand, modelRows] of Object.entries(byBrand)) {
      lines.push(`\n${brand}:`);
      for (const mr of modelRows) {
        const model = mr.model_channel_name || mr.model_channel || mr.model || "?";
        const mentions = mr.mention_count ?? "?";
        const pos = mr.avg_position != null ? mr.avg_position.toFixed(1) : "?";
        const sov = mr.share_of_voice != null ? `${(mr.share_of_voice * 100).toFixed(1)}% SoV` : "";
        lines.push(`  ${model}: ${mentions} mentions · avg pos ${pos} ${sov}`);
      }
    }
  } else {
    for (const row of rows) {
      const brand = row.brand_name || row.brand || "Unknown";
      const mentions = row.mention_count ?? "?";
      const pos = row.avg_position != null ? row.avg_position.toFixed(1) : "?";
      const sov = row.share_of_voice != null ? `${(row.share_of_voice * 100).toFixed(1)}%` : "?";
      const sentiment = row.sentiment != null
        ? `${(((row.sentiment_sum / row.sentiment_count) / 2 + 0.5) * 100).toFixed(0)}% positive`
        : "";
      lines.push(`  ${brand}: ${mentions} mentions · avg pos ${pos} · SoV ${sov} ${sentiment ? `· ${sentiment}` : ""}`);
    }
  }
  return lines.join("\n");
}
...
// Backlink authority comparison via DataForSEO — use to show the link gap
// between mrpaint.com.au and competitors.

const DOMAIN_METRICS_TOOL = {
  name: "get_domain_metrics",
  description:
    "Get backlink authority metrics for any domain — backlink count, referring domains, domain rank (0-100), and top anchor texts. Use to compare mrpaint.com.au against competitors: 'how do our backlinks compare?', 'why are they outranking us?', 'what's their domain authority?', 'who's linking to them?'. Pass one or more domains. Always include mrpaint.com.au in the list so the comparison is meaningful.",
  input_schema: {
    type: "object",
    properties: {
      domains: {
        type: "array",
        items: { type: "string" },
        description: "List of domains to analyse, e.g. ['mrpaint.com.au', 'cairnspaintingcontractors.com', 'petespainting.com.au']. Strip https:// and trailing slashes.",
      },
    },
    required: ["domains"],
  },
...
async function runGetDomainMetrics({ domains }, ctx) {
  ...
  if (!login || !password) return "DataForSEO credentials not set — DATAFORSEO_LOGIN / DATAFORSEO_PASSWORD missing.";

  const auth = "Basic " + Buffer.from(`${login}:${password}`).toString("base64");
  const targets = (domains || []).slice(0, 5).map(d => d.replace(/^https?:\/\//i, "").replace(/\/.*$/, "").toLowerCase());

  const results = await Promise.all(targets.map(async (domain) => {
    try {
      ...
      if (!resp.ok) return { domain, error: `HTTP ${resp.status}` };
      const data = await resp.json();
      if (data.status_code !== 20000) return { domain, error: `DFS ${data.status_code}` };
      ...
      return {
        ...
        dofollow_domains: r.referring_domains_dofollow ?? 0,
        top_anchors: (r.anchors || []).slice(0, 5).map(a => a.anchor || a),
      };
    } catch (err) {
      return { domain, error: String(err.message) };
    }
  }));

  const lines = ["Domain authority comparison (DataForSEO rank = 0-100, higher is stronger):"];
  for (const r of results) {
    if (r.error) { lines.push(`\n${r.domain}: ERROR — ${r.error}`); continue; }
    lines.push(
      `\n${r.domain}:`,
      `  Rank: ${r.rank ?? "?"}/100 | Backlinks: ${r.backlinks} | Referring domains: ${r.referring_domains} | Dofollow domains: ${r.dofollow_domains}`,
      r.top_anchors?.length ? `  Top anchors: ${r.top_anchors.join(", ")}` : ""
    );
  }
  return lines.filter(Boolean).join("\n");
}

// ─── tool: publish_job_to_suburb ─────────────────────────────────────────

const PUBLISH_JOB_TOOL = {
  name: "publish_job_to_suburb",
  description:
    "Take a captured job (from the jobs table) and draft + commit a 'Recent jobs' entry to the matching /areas/[suburb] page on the website, AND draft a Google Business Profile post for Nick to publish manually. Use AFTER the user has confirmed they want the job published — never call on the first message. The website change goes via the normal branch → Vercel preview → YES/NO approval flow (so Adrian eyeballs the preview before merge). The GBP post fires to Slack on approval with a 24h SLA for Nick to action manually (Google Business Profile API doesn't allow direct posting). Returns: branch name, preview URL, summary of what was drafted.",
  input_schema: {
    type: "object",
    properties: {
      job_id: { type: "string", description: "ID of the job to publish (from the jobs table). If 'latest' is passed, the most recently captured job for this client is used." },
    },
    required: ["job_id"],
  },
};

async function runPublishJob({ job_id }, ctx) {
  const { ghGetContents, ghCommitMulti, findVercelPreviewUrl, GITHUB_REPO } = require("./github-bot.js");
  const { generateJobContent } = require("./job-publisher.js");

  // 1. Resolve the job.
  let job;
  if (!job_id || job_id === "latest") {
    const { data } = await supa()
      .from("jobs")
      .select("id, suburb, summary, raw_transcript, structured_facts, captured_at, status")
      .eq("client_id", ctx.clientId)
      .order("captured_at", { ascending: false })
      .limit(1);
    job = data?.[0];
  ...
  if (!job) return "No job found to publish.";
  if (!job.suburb) return `Job ${job.id?.slice(0, 8)} has no suburb on it — can't pick a page to publish to. Ask the user which suburb it was in, update the job, then try again.`;

  // 2. Read current locations.json from main + find the matching suburb.
  const file = await ghGetContents("_data/locations.json", "main");
  const locations = JSON.parse(Buffer.from(file.content, "base64").toString("utf-8"));
  const matchSuburb = (s) => String(s || "").toLowerCase().trim().replace(/\s+/g, "-");
  const wantedSlug = matchSuburb(job.suburb);
  const suburbIdx = locations.findIndex(
    (l) => matchSuburb(l.name) === wantedSlug || matchSuburb(l.slug) === wantedSlug
  );
  if (suburbIdx === -1) {
    const known = locations.map((l) => l.name).join(", ");
    return `Job's suburb "${job.suburb}" doesn't match any known suburb on the site. Known: ${known}. Either edit the job's suburb to one of these, or have Nick add the suburb to _data/locations.json first.`;
  }
  const suburb = locations[suburbIdx];

  // 3. Generate suburb-page entry + GBP draft via Sonnet.
  let content;
  try {
    content = await generateJobContent({ job, suburbCtx: suburb });
  } catch (err) {
    return `Content generation failed: ${String(err.message || err)}`;
  }

  // 4. Prepend new entry to recent_jobs[], cap at 6.
  ...
  // Replace if same job_id already there (re-publish), else prepend.
  const filtered = current.filter((e) => e.id !== job.id);
  suburb.recent_jobs = [newEntry, ...filtered].slice(0, 6);
  locations[suburbIdx] = suburb;

  // 5. Commit on a feature branch.
  const ts = Date.now();
  const branch = `bot/${ts}-job-${suburb.slug}`.slice(0, 200);
  let sha;
  try {
    sha = await ghCommitMulti({
      branch,
      baseRef: "main",
      message: `Bot: add recent job to ${suburb.name} — ${content.title.slice(0, 60)}`,
      files: [
        { path: "_data/locations.json", content: JSON.stringify(locations, null, 2) + "\n", encoding: "utf-8" },
      ],
    });
  } catch (err) {
    return `GitHub commit failed: ${String(err.message || err)}`;
  }

  // 6. Stash the pending GBP draft on the job so handleApprove can fire it
  //    to Slack after the merge. Also mark status.
  const pending_publish = {
    branch,
    sha,
    suburb_slug: suburb.slug,
    suburb_name: suburb.name,
    gbp_text: content.gbp_text,
  ...
  try {
    await supa()
      ...
      .update({
        ...
        structured_facts: { ...(job.structured_facts || {}), pending_publish },
      })
      .eq("id", job.id);
  } catch (err) {
    console.warn("[publish_job] couldn't update job status:", err.message || err);
  }

  // 7. Poll for the Vercel preview URL (up to 90s).
  let previewUrl = null;
  try {
    previewUrl = await findVercelPreviewUrl({ commitSha: sha });
  } catch (err) {
    console.warn("[publish_job] preview poll failed:", err.message || err);
  }

  // 8. Return a structured summary for the strategist to relay.
  const lines = [
    `Drafted entry for ${suburb.name}:`,
    `  Title: ${content.title}`,
    `  Body: ${content.body.split(/\s+/).length} words`,
    `  GBP draft: ${content.gbp_text.length} chars (queued for Nick on approval)`,
    "",
    `Branch: ${branch}`,
    `Diff: https://github.com/${GITHUB_REPO}/commit/${sha}`,
    previewUrl ? `Preview: ${previewUrl}` : `Preview building — usually ready in 30-60s.`,
    "",
    `Reply YES to publish (web entry merges + GBP draft fires to Slack for Nick), NO to discard.`,
  ];
  return lines.join("\n");
}
...
const TOOLS = [
  ...
  { def: FLAG_FOR_NICK_TOOL,          run: runFlagForNick },
  ...
  { def: PEEC_VISIBILITY_TOOL,         run: runGetPeecVisibility },
  { def: READ_SITE_PAGE_TOOL,         run: runReadSitePage },
  { def: DOMAIN_METRICS_TOOL,         run: runGetDomainMetrics },
  { def: PUBLISH_JOB_TOOL,            run: runPublishJob },
];
