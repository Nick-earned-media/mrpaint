...
async function runGetLiveSerp({ keyword }, _ctx) {
  if (!keyword) return "Need a keyword.";
  const { getLiveSerp } = require("./dataforseo.js");
  const r = await getLiveSerp(keyword, _ctx);
  if (!r.ok) return `Live SERP fetch failed: ${r.error}`;
  const lines = [`Live Google SERP for "${keyword}" (Australia, en, top 10 + local pack):`];
  ...
  if (r.organic.length > 0) {
    ...
    r.organic.forEach((o) => {
      lines.push(`  ${o.position}. ${o.domain}`);
      lines.push(`     ${o.title}`);
