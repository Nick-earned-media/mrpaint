...
const READ_SITE_PAGE_TOOL = {
  name: "read_site_page",
  description:
    "Fetch and read the actual text content of a page on the client's live website so you can analyse what's there — headings, body copy, calls to action, keyword usage — and give specific (not guessed) recommendations. Use when the user asks 'what's on my [page]?', 'how can I improve [page]?', 'is [page] optimised for [keyword]?', or any time you'd otherwise say 'probably' about page content. Pass the path (e.g. '/industrial-painting/', '/painter-cairns/', '/') and optionally a keyword to optimise for.",
  input_schema: {
    type: "object",
    properties: {
      path: {
        type: "string",
        description: "The site path to fetch, e.g. '/industrial-painting/' or '/painter-cairns/'. Always include leading and trailing slashes.",
      },
      keyword: {
        type: "string",
        description: "Optional — the keyword or topic to audit the page against (e.g. 'industrial painter Cairns'). If provided, the analysis notes keyword density and missing usage.",
      },
    },
    required: ["path"],
  },
...
async function runReadSitePage({ path, keyword }, ctx) {
  const base = process.env.PUBLIC_BASE_URL || "https://mrpaint.com.au";
