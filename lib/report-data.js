...
async function getReportData({
  ...
  const ai_visibility = buildAiVisibility({ semrush, trackedKeywords });
...
async function loadKnownCompetitors(clientId) {
  const { MAX_COMPETITORS } = require("./competitor-filter.js");
  const { data, error } = await supa()
    .from("competitors")
...
function buildCompetitors({ semrush, knownCompetitors, domain }) {
  ...
  const { filterCompetitors, normalizeDomain, isBrandAggregator } = require("./competitor-filter.js");

  const rawRows = Object.values(semrushData).map((c) => {
  ...
  // Two layers: whitelist matches (the competitors Adrian explicitly tracks)
  // AND top auto-detected from Semrush (the actual competitive landscape).
  // If we did whitelist-only, we'd strip all of Semrush's discovered rivals
  // whenever the whitelist names don't ranking for the same keywords yet —
  // which is what was happening, leaving the report with only MrPaint.
  const whitelistRows = filterCompetitors(rawRows, knownCompetitors);
  const knownDomains = new Set(
    (Array.isArray(knownCompetitors) ? knownCompetitors : [])
      .map((c) => c?.domain ? normalizeDomain(c.domain) : null)
      .filter(Boolean)
  );
  const autoDetected = rawRows
    .filter((r) => !isBrandAggregator(r.domain))
    .filter((r) => !knownDomains.has(normalizeDomain(r.domain)));
  const rows = [...whitelistRows, ...autoDetected];

  // Find the "you" row (or compute from semrush.trackingOverview).
  // Both shareOfVoice and competitor Sov from Semrush are already in
  // percentage units (1.20 = 1.20%), so no division by 100 here — otherwise
  // MrPaint's row gets pushed to the bottom of the sort and the renderer
  // shows the wrong leader.
  const youRow = {
    name: "MrPaint",
    domain,
    sov: semrush?.trackingOverview?.shareOfVoice || 0,
    is_you: true,
  };
  ...
  return top.map((c) => ({
    ...c,
    // sov is already a percentage value (e.g. 1.20 = 1.20%), so format directly.
    sov_pct: c.sov != null ? c.sov.toFixed(2) + "%" : "—",
    bar_pct: c.sov != null ? Math.round((c.sov / maxSov) * 100) : 0,
  }));
}

// ─── AI visibility — real data from search-gpt + gemini campaigns ────────

function buildAiVisibility({ semrush, trackedKeywords }) {
  // Pull from the AI engine campaigns we now collect in runSemrushSnapshot.
  // Each entry has { engine, device, overview: { visibility, top10, top3, all, total, ... } }.
  const engines = Array.isArray(semrush?.aiEngines) ? semrush.aiEngines : [];
  if (!engines.length) {
    return {
      note: "No AI engine campaigns found on the Semrush project.",
      engines: [],
      total_tracked: trackedKeywords.length,
    };
  }

  const summarised = engines.map((e) => {
    const ov = e.overview || {};
    return {
      engine: prettyEngineName(e.engine),
      engine_raw: e.engine,
      device: e.device,
      visibility: ov.visibility != null ? `${Number(ov.visibility).toFixed(2)}%` : "—",
      visibility_delta: ov.differenceVisibility != null
        ? `${ov.differenceVisibility >= 0 ? "▲" : "▼"} ${Math.abs(ov.differenceVisibility).toFixed(2)}%`
        : null,
      keywords_tracked: ov.total ?? null,
      keywords_ranked: ov.all ?? null,
      top3: ov.top3 ?? null,
      top10: ov.top10 ?? null,
      error: ov.error || null,
    };
...
1. **narrative**: 2 short paragraphs for the "What happened" card. First paragraph: what moved this week (use specific keywords and numbers from the data). Second paragraph: where the competitive picture sits + any AI-visibility signal worth flagging.
   ...
   IMPORTANT — competitor mentions: only name competitors that appear in 'competitors_top' below. Never name a brand or business that isn't in that list, even if you recognise it. If 'competitors_top' is empty, say something like "no direct competitors logged yet" — don't invent or guess.
...
async function synthesizeNarrative(payload) {
  ...
  if (!apiKey) {
    console.warn("synthesizeNarrative: ANTHROPIC_API_KEY not in process.env at call time");
...
function extractJson(text) {
  ...
  throw new Error(`no parseable JSON found in: ${trimmed.slice(0, 200)}`);
}
