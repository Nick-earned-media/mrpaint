...
async function getOrganicKeywords(target, token) {
  ...
  // Ahrefs v3 column name for the keyword's best ranking position is
  // unusually `best_position_set` (the `_set` suffix is part of the column
  // name, mirroring the bucket booleans like `is_best_position_set_top_3`).
  url.searchParams.set("select", "keyword,best_position_set,volume,kd,traffic");
