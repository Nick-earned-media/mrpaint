...
function renderReport(data) {
  const { client, scope, period, kpis, movers, all_keywords, competitors, ai_visibility, narrative, actions } = data;
  ...
  // "Improve report" button — opens WhatsApp with a prefilled message that
  // triggers the strategist's report-feedback interview flow (see
  // lib/strategist-prompt.md "When the user wants to improve the report").
  const askPath = "improve report";
...
${(ai_visibility?.engines || []).length ? `
<section class="card">
  <div class="eyebrow">AI search visibility</div>
  <h2 style="margin-bottom:14px">Showing up in ChatGPT & Gemini</h2>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
    ${ai_visibility.engines.map((e) => `
      <div style="border:1px solid var(--line);border-radius:10px;padding:14px;background:#fff">
        <div style="font-size:12px;letter-spacing:.1em;text-transform:uppercase;color:var(--muted);font-weight:700">${escapeHtml(e.engine)}</div>
        <div style="font-size:24px;font-weight:800;margin-top:6px;color:var(--navy)">${escapeHtml(e.visibility)}</div>
        ${e.visibility_delta ? `<div style="font-size:13px;color:var(--ink-2);margin-top:2px">${escapeHtml(e.visibility_delta)}</div>` : ""}
        <div style="font-size:13px;color:var(--ink-2);margin-top:10px">
          <strong>${escapeHtml(String(e.top10 ?? "—"))}</strong> in top 10
          ${e.top3 != null ? ` · <strong>${escapeHtml(String(e.top3))}</strong> in top 3` : ""}
        </div>
        <div style="font-size:12px;color:var(--muted);margin-top:4px">${escapeHtml(String(e.keywords_tracked ?? "—"))} keywords tracked</div>
      </div>
    `).join("")}
  </div>
  <p style="margin-top: 12px; font-size: 13px; color: var(--muted)">Live data from Semrush AI search campaigns. Visibility = % of total possible exposure across tracked keywords.</p>
</section>
` : ""}
...
<nav class="ctabar" aria-label="Quick actions">
  <a class="btn btn--primary" href="${escapeHtml(askDeepLink)}" rel="noopener">✨ Improve report</a>
