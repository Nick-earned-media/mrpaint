...
async function getLiveSerp(keyword, ctx) {
  const auth = dfsAuth();
  ...
  try {
    const resp = await fetch("https://api.dataforseo.com/v3/serp/google/organic/live/advanced", {
      ...
      body: JSON.stringify([{
        keyword,
        location_code: AU_LOCATION_CODE,
    ...
    await logUsage("live_serp", ctx, { keyword });
    return {
      ok: true,
      keyword,
...
async function getAIOverview(keyword, clientDomain, ctx) {
  const auth = dfsAuth();
  ...
  try {
    ...
    await logUsage("ai_overview", ctx, { keyword });
    if (!aiItem) {
      return { ok: true, keyword, present: false };
    }
