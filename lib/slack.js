...
function notifyGbpPost({ clientName, suburb, jobTitle, gbpText, previewUrl, imageUrl, photoAlt }) {
  // Build a Markdown summary block (for clients that don't render Block Kit
  // images) and a header line.
  const summaryLines = [
    ...
    imageUrl ? `_Photo: ${imageUrl}_` : null,
    "",
    "_Action needed: paste into Google Business Profile within 24h. Attach the photo below._",
  ].filter(Boolean);
  const text = summaryLines.join("\n");

  if (!imageUrl) return postToSlack({ text });

  // With image: render the photo inline so Nick can right-click → save
  ...
  // Always pass `text` as the fallback (for mobile notifications, etc.).
