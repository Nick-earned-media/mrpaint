...
function notifyGbpPost({ clientName, suburb, jobTitle, gbpText, previewUrl }) {
  const lines = [
    ...
    "",
    "_Action needed: paste into Google Business Profile within 24h. Attach the job photo if Adrian sent one._",
  ].filter(Boolean);
  return postToSlack({ text: lines.join("\n") });
}
